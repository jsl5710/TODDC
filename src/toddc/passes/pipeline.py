...
def run_dialogue(*, dialogue_id: str, window: list[DialogueTurn], operators: list[Operator],
                 ...
                 judge=None, judge_pool=None, seed: int = 0,
                 control_multiplier: int = 1) -> list[Record]:
    ...
    (coherent-paraphrase) site — each with its own chain seed, so a live sampling
    model produces different wordings — growing the negative (coherent) class
    toward the positives for class balance. See toddc.balancing."""
    ...
    plan: list[tuple[Site, int]] = []
    for site in sites:
        if control_multiplier > 1 and site.operator.family == "control":
