...
def plan_run(raw_dialogues: Optional[list[dict[str, Any]]] = None, *,
    ...
    from toddc import balancing
    from toddc.positioning import enumerate_sites, select_sites
    ...
    for raw in raw_dialogues:
        d = parse_dialogue(raw)
        sites = select_sites(enumerate_sites(d.turns, ops), policy=policy, seed=seed)
        for s in sites:
            if s.operator.family == "control":
                control_units += mult
            else:
                units += 1
