...
def test_auto_control_multiplier_from_operator_mix():
    ops = all_operators()
    m = balancing.auto_control_multiplier(ops, lambda o: o.family == "control")
    n_ctrl = sum(1 for o in ops if o.family == "control")
...
def test_control_multiplier_grows_negative_class():
    ...
    kw = dict(dialogue_id=d.dialogue_id, window=d.turns,
    ...
    neg_base = sum(1 for r in base if not r.gold.should_flag)
    neg_grown = sum(1 for r in grown if not r.gold.should_flag)
    pos_base = sum(1 for r in base if r.gold.should_flag)
    pos_grown = sum(1 for r in grown if r.gold.should_flag)
...
def test_generate_seed_writes_balanced_and_full(tmp_path):
    ...
    import json
    pos = sum(1 for ln in balanced if json.loads(ln)["gold"]["should_flag"])
