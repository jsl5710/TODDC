...
from toddc.runners.factory import check_config, check_spec, planned_split

_CFG = textwrap.dedent("""
    generators:
      - {adapter: toddc.runners.open_source:VLLMClient, model_id: qwen, endpoint: http://127.0.0.1:1/v1}
      - {adapter: toddc.runners.open_source:VLLMClient, model_id: llama, endpoint: http://127.0.0.1:1/v1}
      - {adapter: toddc.runners.open_source:VLLMClient, model_id: mistral, endpoint: http://127.0.0.1:1/v1}
    generation: {split: round_robin, seed: 0}
    judges:
      - {adapter: toddc.runners.open_source:VLLMClient, model_id: judge-a, endpoint: http://127.0.0.1:1/v1}
      - {adapter: toddc.runners.openai:OpenAIClient, model_id: gpt-4o, api_key_env: OPENAI_API_KEY}
...
def test_planned_split_is_even(tmp_path):
    ...
    assert split == {"qwen": 33, "llama": 33, "mistral": 33}
    assert planned_split(10, "generators", "generation", config_path=cfg) == \
        {"qwen": 4, "llama": 3, "mistral": 3}
...
def test_check_config_flags_unreachable_and_missing_key(tmp_path):
    ...
    assert all(not c["ok"] for c in rep["generators"])
...
def test_check_spec_closed_key_present(monkeypatch):
    ...
    c = check_spec({"adapter": "toddc.runners.claude:ClaudeClient",
